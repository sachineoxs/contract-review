...
async function fetchSalesOrderPdfFromOdoo(
  ...
  try {
    ...
    console.log(`SERVER_ACTION: ERP XML-RPC authentication successful for SO fetch. UID: ${uid}`);
  } catch (authError: any) {
    console.error('SERVER_ACTION: ERP XML-RPC Authentication Error (SO Fetch):', authError.message);
    throw new Error(`ERP Authentication Failed (XML-RPC for SO): ${authError.message}. Please check server credentials.`);
  }
  ...
  try {
    sales = await new Promise((resolve, reject) => {
      objectClient.methodCall('execute_kw', [
        ...
        [[['name', '=', soUserInputName]]],
        { fields: ['id', 'name'] }
      ], (error, value) => {
        if (error) return reject(new Error(`ERP XML-RPC search failed: ${error.message}.`));
        resolve(value || []);
      });
    });
    console.log(`SERVER_ACTION: ERP XML-RPC search completed. Found ${sales.length} sales orders.`);
  } catch (searchError: any) {
    console.error('SERVER_ACTION: ERP XML-RPC Search Error:', searchError.message);
    throw new Error(`ERP Search Failed: ${searchError.message}. Please check server credentials.`);
  }

  if (!sales || sales.length === 0) {
    throw new Error(`Sales Order '${soUserInputName}' not found in the system.`);
  }
  ...
  if (startOfFile.includes('<!doctype html') || startOfFile.includes('<html')) {
    console.error(`SERVER_ACTION: Failed to fetch PDF for SO '${actualSaleOrderName}'. The system returned an HTML page instead of a PDF. This typically indicates a login, session, or permissions issue.`);
...
export async function compareOrdersAction(
  ...
  if (!odooUrl || !odooDb || !odooUsername || !odooPassword) {
    console.error("SERVER_ACTION: ERP connection details are not configured on the server.");
    return { error: 'ERP connection details are not configured on the server.' };
  }
